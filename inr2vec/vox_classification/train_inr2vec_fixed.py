...
class InrDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, Tensor, Tensor]:
        pid = os.getpid()
        if pid not in self.file_handles:
            # Open a new file handle for this process
            self.file_handles[pid] = h5py.File(self.mlps_paths[index], "r")
        
        f = self.file_handles[pid]
...
class Inr2vecTrainer:
    ...
    def plot(self, split: str) -> None:
        ...
        for i in range(bs):
            ...
            #threshold = min(adaptive_threshold, 0.4)  # Ensure threshold is not too high
